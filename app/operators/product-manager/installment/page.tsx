...
function InnerComponent() {
  ...
  // Two-stage PRODUCT step (BUY_INSTALLMENT): the same page renders both
  // PRODUCT stages; the step detail API exposes step.isFinal which tells us
    ...
    fetch(`/api/workflow/step/${stepInstanceId}`, { credentials: "include" })
      ...
      .then((data) => {
        ...
        setInitialData({
          ...
          statusType: dry,
          // BUY_INSTALLMENT stores the sim price in `sp`
          customerPrice: inst.sp || inst.dPrice || inst.mPrice || inst.price || "—",
        });
      });
  }, [stepInstanceId]);
  ...
  return (
    <RoleWorkflowPage
      stepTitle={isFinalStep ? "بررسی نهایی محصول (اقساطی)" : "بررسی محصول (اقساطی)"}
      displayFields={["BuyDesiredNumber"]}
      inputFields={["isAvailable", "hasInstallmentOption", "ProductPrice", "Avablity", "agentNote"]}
